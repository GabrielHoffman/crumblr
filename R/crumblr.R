...
setMethod(
  ...
  function(counts, pseudocount = 0.5, method = c("clr", "clr_2class"), tau = 1) {
    ...
    if (method == "clr") {
      res <- .crumblr(counts, pseudocount, tau)
    } else {
...
setMethod(
  ...
  function(counts, pseudocount = 0.5, method = c("clr", "clr_2class"), tau = 1) {
    ...
    if (method == "clr") {
      res <- .crumblr(as.matrix(counts), pseudocount, tau)
    } else {
      res <- .clr_2class(as.matrix(counts), pseudocount)
    }
    res
  }
)
...
.clr_2class <- function(counts, pseudocount = 0.5) {
