...
run_sim = function(other, pseudocount = .5){
	...
	df_res = mclapply( c(300, 1000), function(countTotal){

		df_res = lapply( seq(1, countTotal - sum(other) - 1 , by=1), function(targetCount){
			...
			df_res = lapply( c(1, 5, 20), function(tau){
				...
				# D = ncol(counts)
				# v_theoretical = tau * (1/p - 2/(p*D) + sum(1/p)/D^2) / countTotal
